...
async def generate_inspection_ai_review_endpoint(
    ...
    try:
        ...
        # ------------------------------------------------------------
        # IMAGE-TO-LLM REVIEW DISABLED
        # ------------------------------------------------------------
        # The maintenance advisory now uses ONLY the stored detector/event/GPS
        # facts. No context/crop images are downloaded from Google Drive and no
        # images are sent to Gemini/Groq for secondary visual inspection.
        #
        # Historical event-level supplementary_visual_review fields may remain
        # in MongoDB for audit compatibility, but this endpoint no longer reads,
        # generates, or depends on them.
        supplementary_visual_summary = {
            "enabled": False,
            "performed_count": 0,
            "reused_existing_count": 0,
            "failed_count": 0,
            "classes_using_images": [],
            "reason": "Image-to-LLM secondary inspection is disabled; advisory uses detector/event/GPS data only.",
        }

        print(
            "AI VISUAL REVIEW DISABLED | "
            f"inspection_id={inspection_id} | "
            f"events={len(event_docs)} | "
            "mode=text_data_only"
        )

        # google-genai and groq SDK calls are synchronous, so run the final
        # text/data-only advisory call in a worker thread.
        ...
        return AIReviewGenerationResponse(
            ...
            supplementary_visual_summary=supplementary_visual_summary,
            ai_advisory=advisory,
            message="Myanmar-language data-only AI maintenance review generated.",
        )
